...
setInterval(refreshData, 250); // this should be less for realtime streaming
...
function streamAudioThroughDataChannel(dataChannel) {
    ...
    var superBlob = null; // Blob with accumulated audio blobs
    
    mediaRecorder.ondataavailable = function(event) {
        if(chunks.length < 64) {
            // doing multiple chunks with large amount of audio data
            // for audio quality testing. Need to send each chunk in realtime
            // and decode it for realtime streaming.
            chunks.push(event.data);
        }
        else {
            superBlob = new Blob(chunks);
            // sends arrayBuffer of audio data stored in blobs generated by
            // MediaRecorder
