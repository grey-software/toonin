<template>
  <q-card class="mx-auto" max-width="900px" :elevation="40">
    <q-card-title class="toonin-title">{{ cardTitle }}</q-card-title>
    <q-container fluid>
      <q-card>
        <q-card-text>
          <q-list ref="chat" id="logs">
            <template q-for="(item, index) in messages">
              <q-subheader q-if="item" :key="index">{{ item }}</q-subheader>
            </template>
          </q-list>
        </q-card-text>
      </q-card>
